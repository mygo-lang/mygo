...
| 模式匹配（通配符、字面量、变体解构） | ✅ 完整（仅 switch 内） | `VariantPattern` + `WildcardPattern` + `LiteralPattern` |
| HM 类型推理（单文件、let 多态） | ⚠️ 基础完整 | 无完整 Instantiate/Generalize，`Scheme.Bound` 未用于量化 |
| 尾递归优化 | ✅ 完整 | — |
...
- **缺少**：`TupleLitExpr`、`UnitLitExpr`、`TuplePattern`、`BindPattern`、`CallExpr.TypeArgs`
...
| Pattern 匹配 | `Pattern`/`BindPattern`（含 `TuplePattern`） | ❌ 仅 `VariantPattern`/`WildcardPattern`/`LiteralPattern` |
...
| TuplePattern | ✅ 完整 `TuplePattern` | ❌ 未实现 |
...
1. **最小化语言子集**：`codegen2` 只实现了 MyGO 功能的最小子集（无完整类型类、无 HKT、无缝合类型类），降低引导负担
2. **紧凑 AST**：`ast2` 去除了位置信息、Pattern 匹配体系等非核心部分，但**保留了独立的 `Stmt` 类型**以简化自举代码
   - **更新**：`ast2` 已添加 `MapLitExpr`、`SetLitExpr`、`Constraint` 结构体，但仍缺少 `TupleLitExpr`、`TuplePattern`、`BindPattern`、`CallExpr.TypeArgs`、`embed` 声明
...
- **`CallExpr.TypeArgs`**：ast2 `CallExpr` 无 `TypeArgs` 字段，parser2 不解析泛型调用类型参数
- **`embed` 声明**：parser2 支持 `embed` 关键字但未解析为 AST 节点
- **`TupleLitExpr`**：ast2 无 `TupleLitExpr` 变体，parser2 的 `tupleOrParenExpr()` 生成 `TupleExpr`
- **`TuplePattern` / `BindPattern`**：模式匹配仅支持 `VariantPattern`，不支持元组模式和绑定模式
