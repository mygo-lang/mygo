...
func TestStructFieldSymbolProbe(t *testing.T) {
	...
	if s, ok := envGet(ti2Env, "ast2.Decl").(OptionSome[Scheme]); ok {
	...
	if s, ok := envGet(cd2Env, "ast2.Decl").(OptionSome[Scheme]); ok {
	...
	for _, d := range ti2Decls {
		if sd, ok := d.(ast2.DeclStructDecl); ok && sd.F0 == "PkgDeclSource" {
			for _, f := range sd.F2 {
				if f.Name == "Decls" {
	...
	for _, s := range manual {
		if ss, ok := s.(SymbolStructField); ok && ss.F0 == "typeinference2.PkgDeclSource" && ss.F1 == "Decls" {
	...
	for loopIdx, lpkg := range packages {
		...
		if lpkg.Alias == "typeinference2" {
			if s, ok := envGet(lpkgEnv, "ast2.Decl").(OptionSome[Scheme]); ok {
	...
	for _, s := range loopSyms {
		if ss, ok := s.(SymbolStructField); ok && ss.F1 == "Decls" {
	...
	for _, s := range symbols {
		if ss, ok := s.(SymbolStructField); ok {
