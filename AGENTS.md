...
- **`prelude_go.go` compiles with generated `Eq[T]`**: The hand-written `prelude_go.go` references the `Eq[T]` interface and `Option[*T]` enum types, which are generated by the compiler in `zz_mygo.gen.go` during compilation. This means `prelude_go.go` only compiles after `zz_mygo.gen.go` has been generated.
- **Prelude generation fails on `optionMap` switch**: The function body of `optionMap` contains a switch expression that produces a block with assignments, and the code generator's `translateBlock` doesn't correctly return the enum constructor type for case body expressions like `Some(fn(v))`. This is a `translateCall`/`translateEnumConstructor` interaction issue — the constructor emits correct Go code but `translateExpr` returns an empty type string, causing `translateBlock` to reject it as missing. A `translateBlock` fallback for pure-statement blocks was added (`rawBlock` path when `!lastWasExprStmt && !sawReturn`), but the root cause in `translateCall`/`translateEnumConstructor` type propagation remains unresolved.
